...
function autocomplete(input, latInput, lngInput) {
  if (!input) return; // skip if address is empty
  ...
  _axios2.default.get("/api/stores/near?lat=" + lat + "&lng=" + lng).then(function (res) {
    ...
    var markers = places.map(function (place) {
      var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
          placesLng = _place$location$coord[0],
          placeLat = _place$location$coord[1];

      var position = { lat: placeLat, lng: placesLng };
    ...
    //zoom the map to fit all the markers perfectlyf
...
function makeMap(mapDiv) {
  ...
  loadPlaces(map);
  var input = (0, _bling.$)('[name="geocomplete"]');
  var autocomplete = new google.maps.places.Autocomplete(input);
  autocomplete.addListener("place_changes", function () {
